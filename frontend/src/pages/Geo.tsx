...
export default function Geo() {
  ...
  const load = () => api.get<GeoMention[]>(`/api/geo-mentions?site_id=${siteId}`).then(setMentions);
  ...
  async function handleRun() {
    ...
    setError(null);
    try {
      await api.post("/api/geo-mentions/run", { site_id: Number(siteId) });
      await load();
    } catch (err) {
  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between">
        <h1 className="text-xl font-semibold">GEO Tracker</h1>
        <button
          className="rounded-md bg-brand-blue px-4 py-2 text-sm font-semibold text-black disabled:opacity-50"
          onClick={handleRun}
          disabled={running}
        >
          {running ? "Checking…" : "Run GEO check now"}
        </button>
      </div>
      <p className="mb-6 text-sm text-slate-500">
        Runs automatically every Monday against every provider with an API key configured (ChatGPT, Claude,
        Gemini, Perplexity), for your top approved keywords.
      </p>
